#include "hw_base_access.H"

#include <transport/transport.H>

namespace hwaccess
{
int HwBaseAccess::getCfam(ConstTargetPtr_t target, uint32_t addr, uint32_t& val)
{
    if(target == nullptr)
    {
        std::cerr << "DEMO HwBaseAccess::getCfam target is nullptr" << std::endl;
        return -1;
    }

    std::cout << "DEMO HwBaseAccess::getCfam executed" << std::endl;
    return transport::getCfam(target, addr, val);
}

int HwBaseAccess::putCfam(ConstTargetPtr_t target, uint32_t addr, uint32_t val)
{
    if(target == nullptr)
    {
        std::cerr << "DEMO HwBaseAccess::putCfam target is nullptr" << std::endl;
        return -1;
    }
    
    std::cout << "DEMO HwBaseAccess::putCfam executed" << std::endl;
    return transport::putCfam(target, addr, val);
}
} //namespace hwaccess
